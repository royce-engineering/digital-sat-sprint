...
function readQuestions(ids: string[]): ExamQuestion[] {
  return ids.map(getQuestion).filter((question): question is ExamQuestion => Boolean(question));
}

function questionDomain(question: ExamQuestion): string {
  return question.blueprint?.domain ?? question.domain ?? "Unclassified Reading & Writing";
}

function questionSkill(question: ExamQuestion): string {
  return question.blueprint?.skill ?? question.skill ?? "Unclassified skill";
}
...
export function saveReadingSessionAttempt(
  ...
  const module1Questions = readQuestions(session.module1QuestionIds);
  const module2Questions = readQuestions(session.module2QuestionIds);
  ...
  const modules: ScoreModule[] = [
    {
      ...
      section: "Reading & Writing",
      difficulty: "Medium",
      questionIds: module1Questions.map((question) => question.id),
    },
    {
      key: "reading-module-2",
      title: "Reading & Writing Module 2",
      section: "Reading & Writing",
      difficulty: session.module2Route ?? "Easy",
      questionIds: module2Questions.map((question) => question.id),
    },
  ];

  const score = estimateSectionScore("Reading & Writing", modules, session.answers);
