  ...
  return (
    <main className="mx-auto max-w-7xl space-y-7 p-5 sm:p-8">
      <section className={`rounded-3xl p-8 text-white ${report.passed ? "bg-emerald-950" : "bg-red-950"}`}>
        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-white/70">Sprint 54E-B</p>
        <div className="mt-4 flex flex-wrap items-end justify-between gap-5">
          <div>
            <h1 className="text-4xl font-bold">Math Production Selector Stress Test</h1>
            <p className="mt-3 text-white/75">
              {report.passed
                ? `All ${report.totalRuns} production generations passed.`
                : `${report.failures.length} blocking failure(s) found.`}
            </p>
          </div>
          <button
            type="button"
      ...
      <section className="grid gap-4 sm:grid-cols-4">
        ...
        <Metric label="Blocking failures" value={report.failures.length} />
      </section>

      <section className="rounded-3xl border bg-white p-6 shadow-sm">
        <h2 className="text-2xl font-bold">Route results</h2>
          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="p-3">Max warnings/run</th>
              </tr>
            </thead>
            <tbody>
              {report.routes.map((route) => (
                <tr key={route.route} className="border-t">
                  ...
                  <td className="p-3">{route.warningCount}</td>
        ...
        <section className="rounded-3xl border border-red-200 bg-red-50 p-6">
          ...
          <div className="mt-5 space-y-3">
            {report.failures.slice(0, 100).map((failure, index) => (
              <article key={`${failure.route}-${failure.seed}-${failure.code}-${index}`} className="rounded-xl bg-white p-4">
                <p className="font-bold">{failure.route} · {failure.code}</p>
                <p className="mt-1 text-sm text-slate-600">Seed: {failure.seed}</p>
