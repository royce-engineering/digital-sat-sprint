...
import { buildMathAdaptiveExam } from "@/lib/adaptive/mathAdaptiveEngine";
...
const MODULE_SECONDS = MATH_MODULE_SECONDS;
...
function createInitialSession(): MathSessionState {
  return {
    version: 1,
    phase: "intro",
    module1Seed: newSeed(),
...
function resolveQuestions(ids: string[]): ExamQuestion[] {
  return ids
    .map((id) => getQuestion(id))
    .filter((question): question is ExamQuestion => Boolean(question));
}

function AdaptiveMathPageContent() {
  const [session, setSession] = useState<MathSessionState | null>(null);

  useEffect(() => {
    setSession(loadMathSession() ?? createInitialSession());
  }, []);

  useEffect(() => {
    if (session) saveMathSession(session);
  }, [session]);

  const adaptiveExam = useMemo(() => {
    if (!session) return null;

    return buildMathAdaptiveExam({
  ...
  const activeQuestions = useMemo(
    () => resolveQuestions(activeQuestionIds),
    [activeQuestionIds],
  );
  ...
  const startExam = () => {
    ...
    setSession({
      ...session,
      phase: "module-1",
      module1QuestionIds: adaptiveExam.module1.map(
        (question) => question.examId,
      ),
      module2QuestionIds: [],
  ...
  if (session.phase === "results") {
    const allQuestions = resolveQuestions([
      ...session.module1QuestionIds,
      ...session.module2QuestionIds,
    ]);
    ...
    return (
      <main className="mx-auto max-w-5xl space-y-6 p-8">
        <section className="rounded-2xl border bg-white p-8 shadow-sm">
          ...
          <h1 className="mt-2 text-3xl font-bold">Math Results</h1>
    ...
    <main className="mx-auto max-w-6xl p-4 sm:p-8">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_290px]">
        <section className="rounded-2xl border bg-white p-6 shadow-sm sm:p-8">
          ...
          <h2 className="mt-6 whitespace-pre-wrap text-lg font-semibold leading-7">
            ...
            {activeQuestions.map((question, index) => {
              ...
              return (
                <button
                  ...
                  className={`aspect-square rounded-lg border text-sm font-semibold ${
...
export default function AdaptiveMathPage() {
  return (
    <SessionLeaseGuard kind="math">
      <AdaptiveMathPageContent />
    </SessionLeaseGuard>
  );
}
