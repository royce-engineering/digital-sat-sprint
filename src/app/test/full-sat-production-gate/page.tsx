...
import { runFullSatProductionGate } from "@/lib/adaptive/fullSatProductionGate";

export default function Page() {
  const [run, setRun] = useState(0);

  const report = runFullSatProductionGate({
    ...
    <main className="mx-auto max-w-6xl space-y-7 p-5 sm:p-8">
      ...
      >
        ...
        <div className="mt-4 flex flex-wrap items-end justify-between gap-5">
          <div>
            <h1 className="text-4xl font-bold">Full SAT Production Gate</h1>
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <Metric label="Production checks" value={report.checks.length} />

        <Metric label="Gate status" value={report.passed ? "PASS" : "FAIL"} />
      </section>

      <section className="rounded-3xl border bg-white p-6 shadow-sm">
        <h2 className="text-2xl font-bold">Release checks</h2>
